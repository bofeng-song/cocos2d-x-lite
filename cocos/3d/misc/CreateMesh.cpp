...
Mesh *createMesh(const IGeometry &geometry, Mesh *out, const ICreateMeshOptions &options /* = {}*/) {
    ...
    if (!positions.empty()) {
        ...
        auto &channelInfo     = channels.back();
        channelInfo.offset    = stride;
        channelInfo.data      = positions;
        channelInfo.attribute = *attr;
        stride += info.size;
    }
    ...
    if (geometry.normals.has_value() && !geometry.normals.value().empty()) {
        ...
        auto &channelInfo     = channels.back();
        channelInfo.offset    = stride;
        channelInfo.data      = geometry.normals.value();
        channelInfo.attribute = *attr;
        stride += info.size;
    }
    ...
    if (geometry.uvs.has_value() && !geometry.uvs.value().empty()) {
        ...
        auto &channelInfo     = channels.back();
        channelInfo.offset    = stride;
        channelInfo.data      = geometry.uvs.value();
        channelInfo.attribute = *attr;
        stride += info.size;
    }
    ...
    if (geometry.tangents.has_value() && !geometry.tangents.value().empty()) {
        ...
        auto &channelInfo     = channels.back();
        channelInfo.offset    = stride;
        channelInfo.data      = geometry.tangents.value();
        channelInfo.attribute = *attr;
        stride += info.size;
    }
    ...
    if (geometry.colors.has_value() && !geometry.colors.value().empty()) {
        ...
        auto &channelInfo     = channels.back();
        channelInfo.offset    = stride;
        channelInfo.data      = geometry.colors.value();
        channelInfo.attribute = *attr;
        stride += info.size;
    }
    ...
    if (geometry.customAttributes.has_value()) {
        for (auto ca : geometry.customAttributes.value()) {
            ...
            auto &channelInfo     = channels.back();
            channelInfo.offset    = stride;
            channelInfo.data      = ca.values;
            channelInfo.attribute = ca.attr;
            stride += info.size;
        }
    }
    ...
    Mesh::IBufferView   vbv;
    vbv.offset              = bufferBlob.getLength();
    vbv.length              = static_cast<uint32_t>(vertexBuffer->byteLength());
    vbv.count               = vertCount;
    vbv.stride              = stride;
    vertexBundle.view       = vbv;
